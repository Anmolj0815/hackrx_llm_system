...
Original file is located at
    https://colab.research.google.com/drive/1Dg_U-8H4Oiyrsf02ufzRpuJH4fsAmTb0
"""
...
async def get_answer_from_llm(question: str, context: str) -> str:
    ...
    try:
        ...
        # More specific handling for common errors
...
async def run_hackrx_query(
    ...
    # Generate a unique ID for the dynamic document of this request
    # This ID will be used for upserting and potentially for temporary namespace management
    current_request_doc_id = str(uuid.uuid4())
    # Optional: Use a request-specific namespace for the dynamic document to keep it separate,
    # and maybe delete this namespace later if Pinecone supports TTL on namespaces or if cleanup is feasible.
    # For now, we'll put dynamic docs into a temporary namespace or just the main one.
    # Using a dedicated namespace for dynamic documents might be cleaner for competition if index is shared
    # between evaluation calls and you want to ensure no cross-contamination from previous evaluation documents.
    dynamic_doc_namespace = f"dynamic-doc-{current_request_doc_id}" # Example of a dedicated namespace
    ...
    try:
        ...
        if not document_text.strip():
            ...
            # Chunk text and upsert to Pinecone into a unique namespace for this request's document
        ...
        for i, question in enumerate(questions):
            ...
            # We query across both the static policies namespace and the current dynamic document's namespace.
            # Pinecone's query can search across multiple namespaces if configured or by combining results.
            # For simplicity, let's query the main index and assume relevant static docs are mixed with dynamic.
            # Or, query two namespaces and combine:
            ...
            # Combine and de-duplicate chunks (simple join for now, could use a re-ranker for better quality)
            # Prioritize dynamic chunks slightly if they're available, or mix them.
            # A more advanced setup might re-rank these combined chunks before sending to LLM.
            combined_chunks = static_relevant_chunks + dynamic_relevant_chunks

            # Remove duplicates by converting to set and back to list (loses order, fine for context)
            combined_chunks = list(dict.fromkeys(combined_chunks)) # Preserves order, removes duplicates

            context = "\n".join(combined_chunks)
            if not context.strip():
                print(f"No relevant context found for question: {question}")
                # Provide a generic "cannot determine" answer if no context is found
        ...
        raise e
    except ValueError as e: # Catch specific value errors (e.g., download failed)
        ...
        # However, deleting by namespace for a short-lived doc might be tricky if not atomic.
        # For simplicity in contest, you might just let it accumulate or clear whole index if needed.
        # If using a unique namespace per request, Pinecone's delete by namespace is efficient.
        try:
            ...
            # Don't fail the API call if cleanup fails
...
async def health_check():
    ...
    # Optionally, also check Pinecone connection
    try:
        if pinecone_index:
            # A simple describe_index_stats can confirm connectivity
